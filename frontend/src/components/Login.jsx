          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="email" placeholder="Correo electrónico" value={email}
              ...
              className="w-full px-4 py-3 bg-white border border-neutral-200 rounded-lg text-sm
                         focus:outline-none focus:border-neutral-400 transition-colors placeholder:text-neutral-300" required />
          ...
          <form onSubmit={handleRegister} className="space-y-3">
              ...
              {['cliente', 'empleado'].map(r => (
                <button key={r} type="button" onClick={() => setRolReg(r)}
                  className={`flex-1 py-2.5 rounded-lg text-sm border transition-colors
                    ${rolReg === r ? 'bg-neutral-900 text-white border-neutral-900' : 'border-neutral-200 text-neutral-500 hover:border-neutral-400'}`}>
            ...
            {rolReg === 'cliente' ? (
              <input type="text" placeholder="Teléfono (opcional)" value={telefono}
                ...
                className="w-full px-4 py-3 bg-white border border-neutral-200 rounded-lg text-sm
                           focus:outline-none focus:border-neutral-400 transition-colors placeholder:text-neutral-300" />
            ) : (
